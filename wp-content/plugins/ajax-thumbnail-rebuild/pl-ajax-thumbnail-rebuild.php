...
/* Plugin name: AJAX Thumbnail Rebuild
   ...
   Author: junkcoder
   Author URI: http://breiti.cc. Modified by Presslabs
   Version: 1.12
   Description: Rebuild thumbnails starting with the 14th of March 2015.
   Max WP Version: 3.9
   Text Domain: pl-ajax-thumbnail-rebuild
	...
	if ($action == "getlist") {
		...
		if ($onlyfeatured) {
			...
			$featured_images = $wpdb->get_results( "SELECT meta_value,{$wpdb->posts}.post_title AS title FROM {$wpdb->postmeta}, {$wpdb->posts}
		                                        WHERE meta_key = '_thumbnail_id' AND {$wpdb->postmeta}.post_id={$wpdb->posts}.ID AND {$wpdb->posts}.post_date_gmt > '2015-03-14 23:59:59'");
			...
			foreach ( $attachments as $attachment ) {
				if ( strtotime( $attachment->post_date_gmt ) > strtotime( '2015-03-14 00:00:00' ) ) {
